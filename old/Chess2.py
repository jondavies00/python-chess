...
class Board:
    ...
    #Get pieces of specified colour
    ...
    #Get coordinates of a specific piece
    def get_coords(self, piece):
        for npiece in self.board:
            if npiece.colour == piece.colour and npiece.symbol == piece.symbol:
...
class Player():
    ...
    def update_move_list(self, board):
        ...
        for piece in pieces:
            move_set = piece.moveset
            for m in move_set:
                x, y = board.get_coords(piece)
                move = x + m[0]
                if not board.is_blocked(m, (x,y), (x+m[0], y+m[1])):
                    possible_squares.append(m[0] + x, m[1] + y)
